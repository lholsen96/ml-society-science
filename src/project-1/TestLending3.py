# -*- coding: utf-8 -*-
"""
Created on Thu Oct  4 13:38:36 2018
...
## Test function
## Here we also include X_test to get the true values of amount and duration
def test_decision_maker(X_test_noise, X_test, y_test, interest_rate, decision_maker):
    n_test_examples = len(X_test)
    utility = 0
    ## Example test function - this is only an unbiased test if the data has not been seen in training
    for t in range(n_test_examples):
        action = decision_maker.get_best_action(X_test_noise.iloc[t])
...
def calculate_utility(n_tests, X, encoded_features, numerical_features, categorical_features, target, decision_maker, interest_rate, noise = False):
    ...
    for iter in range(n_tests):
        ...
        if noise == True: 
            X_test_noise = add_noise(X_test, numerical_features, categorical_features)
        utility += test_decision_maker(X_test_noise, X_test, y_test, interest_rate, decision_maker)
    return utility    
...
## We test the three different policies.
