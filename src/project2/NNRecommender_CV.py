...
class NNRecommender:
    ...
    def fit_treatment_outcome(self, data, actions, outcome):
        ...
        for i in range(max_first_layer):
            for j in range(max_second_layer):  
                print("Cross validation, nodes in first layer is %d, nodes in second layer is %d" %(i, j))
                models.append(MLPClassifier(solver = 'lbfgs', hidden_layer_sizes = (i+2,j+2), random_state = 1))
                cv_scores.append(cross_val_score(models[-1], X = data2, y = np.ravel(self.outcome)))
        ...
        model_index = np.argmin(cv_scores)
    ...
    def estimate_utility(self, data, actions=None, outcome=None, policy=None):
        if (policy == None):
